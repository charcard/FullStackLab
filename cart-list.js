...
cartList.post("/cart-items", (req, res) => {    
  pool.query("INSERT INTO ShoppingCart(product, price, quantity) VALUES($1::text, $2::smallint, $3::smallint)", [request.body.product, request.body.price, request.body.quantity]).then(() => {
      pool.query("SELECT * FROM ShoppingCart").then((result) => {
          res.send(result.rows);
      });    
  });
...
cartList.put("/cart-items/:id", (req, res) => {
  pool.query("UPDATE ShoppingCart SET product=$1::text, price=$2::smallint, quantity=$3::smallint, WHERE id=$4::int", [req.body.product, req.body.price, req.body.quantity]).then(() => {
      pool.query("SELECT * FROM ShoppingCart ORDER BY id").then((result) => {
          res.send(result.rows);
      }); 
  })
